...
public:
    ...
    void renderModel(const BaseModel &model, const Shader &shader, const int &index);
    inline int getIndexByPosition(const int &x, const int &y) const { return static_cast<int>(objectsBuffer[y][x]); }
    void clearFrame();
private:
    void renderTriangle(std::vector<Point<3, double>> &triangle, const char &objectIndex);
